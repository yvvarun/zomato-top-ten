...
class zomato:
    ...
    def __init__ (self, user_key):
        ...
        if len(user_key) == 0:
            self.user_key = "341dd3992c439c8eb7e9c2ba09eb8145"
        else:
        ...
        """
        self.collection_dict = {1:"Best Bars & Pubs", 2:"Microbreweries",\
    ...
    def __check_response_status (self, response):
        ...
        if status_code == 200:
            ...
            print('invalid user key: %s'%self.user_key)
            return -1

    def __populate_table (self, res):
        self.table.field_names = ["Restaurant name", "Rating", "Votes"]
        for r in res['restaurants']:
            ...
            self.table.add_row([name + ", " + location, rating, votes])
        self.table.sortby = "Rating"
    ...
    def get_city_id (self, city_name):
        try:
            ...
            r = requests.get(self.base_url + "cities?q=" + city_name,\
                    headers=headers)
    ...
    def get_collection_id (self, city_name, collection_option):
        try:
            ...
            collection_id = -1
            headers = {'Accept': 'application/json', 'user-key': self.user_key}
            r = requests.get(self.base_url + "collections?city_id=" + str(city_id),\
                    headers=headers)
            status = self.__check_response_status(r)
            if status == 0:
                res = json.loads(r.content.decode("utf-8"))
                if len(res['collections']) > 0:
                    for c in res['collections']:
                        if c['collection']['title'].title() == self.collection_dict[collection_option].title():
                ...
                if len(res['restaurants']) > 0:
                    ...
                    if collection_option == 1:
                        ...
                        if status == 0:
                            ...
                            if len(res['restaurants']) > 0:
                                self.__populate_table(res)
                    print(self.table.get_string(start=0,end=10))
        except Exception as e:
            exc_type, exc_value, exc_traceback = sys.exc_info()
            print("Exception traceback: %s"%str(traceback.format_exception\
                (exc_type, exc_value, exc_traceback)))
